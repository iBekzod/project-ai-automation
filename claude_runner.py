...
There is a Graphify knowledge graph of the CRM repo — 13k nodes, 33k edges — and the `codebase-graph` skill explains how to use it. Ask it "what does changing X affect" before touching anything with dependents; it found a caller that a grep-based search had missed, and the change shipped incomplete because of it. Refresh it with `graphify/refresh.py` after changing code or pulling someone else's — it rebuilds only what moved and returns in a second when nothing did. A stale graph answers confidently and wrongly, which is the worst failure mode available here. The graph knows STRUCTURE; the memory repo knows WHY. Neither replaces the other.
